...
def split_col_types(df):
    ...
    return {
        "categorical": df.select_dtypes(include=categorical_types)
        .columns.unique()
        .tolist(),
...
def plot_ppscore_heatmap(ppscores, figsize=(15, 15)):
    ...
    """
    ppscores = (
        ppscores[["x", "y", "ppscore"]].pivot(columns="x", index="y", values="ppscore")
        * 100
    ).astype(int)
    ...
    ax = sns.heatmap(
        ppscores, cmap="Blues", linewidths=0.5, square=True, annot=True, fmt="g"
    )
...
def get_ppscore_feature_relevante(
    ppscores, target_variable, cols=None, upper_threshold=None
):
    """
    ...
    ppscores = (
        ppscores[(ppscores["x"] != ppscores["y"]) & (ppscores["y"] == target_variable)]
...
def find_redundant_features(ppscores, target_variable, upper_bound_threshold=0.8):
    ...
    xy_scores = ppscores.copy()[
        (ppscores["x"] != ppscores["y"]) & (ppscores["y"] == target_variable)
    ].sort_values(by="ppscore", ascending=False)

    # ppscore from A feature to B feature
    ab_ppscores = ppscores.copy().rename(
        columns={"x": "A", "y": "B", "ppscore": "AB_ppscore"}
    )

    # ppscore from B feature to A feature
    ba_ppscores = ppscores.copy().rename(
        columns={"x": "B", "y": "A", "ppscore": "BA_ppscore"}
    )

    async_ppscores = (
        ab_ppscores.merge(ba_ppscores, how="left", on=["A", "B"])
        .merge(
            xy_scores[["x", "ppscore"]].rename(
                columns={"x": "A", "ppscore": "AY_ppscore"}
            ),
            how="left",
            on=["A"],
        )
        .merge(
            xy_scores[["x", "ppscore"]].rename(
                columns={"x": "B", "ppscore": "BY_ppscore"}
            ),
            how="left",
            on=["B"],
        )
...
def conf_interval(data, num_boot_samples=1000, seed=42, estimator=np.mean):
    ...
    # Bootstrap
    means = [
        estimator(data.sample(len(data), replace=True)) for _ in range(num_boot_samples)
    ]
...
def identify_outliers(df, cols=None):
    ...
    df["is_outlier"] = (
        df["is_outlier_knn"] + df["is_outlier_pca"] + df["is_outlier_iforest"]
    ) > 1
...
def get_features_variance(df, feature_cols=None, upper_threshold=0):
    """
    Select features by variance
    """
    feature_cols = feature_cols if feature_cols else df.columns
    out = df[feature_cols].var()
    out = out[out > upper_threshold]
    out = out.reset_index()
